...
class ArtworkPageTests(unittest.TestCase):
    ...
    def test_image_lightbox_is_enabled_for_every_artwork(self):
        for item in self.catalog["items"]:
            with self.subTest(item=item["id"]):
                page = render_detail_page(self.site, item, self.categories[item["category"]])
                self.assertIn('data-image-lightbox="true"', page)
                self.assertIn('data-lightbox-dialog', page)
                self.assertIn('data-lightbox-size', page)
